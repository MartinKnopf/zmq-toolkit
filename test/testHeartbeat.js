...
describe('[testHeartbeat.js] Heartbeat', function() {
  ...
  describe('When JSON data is given', function() {
  ...
  describe('When String data is given', function() {
  ...
  describe('When no data is given', function() {
    ...
    it('should emit heartbeat without data', function(done) {
      var broker = new Broker().start('tcp://127.0.0.1:9300', 'tcp://127.0.0.1:9301')
        ,  sub = zmq.socket('sub');
      sub.subscribe('heartbeat');
      sub.on('message', function(msg) {
        assert.equal(toJson(msg), undefined);
        done();
        broker.close();
      });
      sub.connect('tcp://127.0.0.1:9301');

      var heartbeat = new Heartbeat().start('tcp://127.0.0.1:9300', 1);
    });
  });
});
